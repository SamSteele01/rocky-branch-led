...
export class Dotstar {
  ...
  constructor(spi: ISpi, options: IDotstarOptions = {}) {
    ...
    const fullBufferLength =
      Dotstar.startBytesLength +
      this.length * Dotstar.bytesPerLed +
      Dotstar.endBytesLength;
    ...
    this.colorBuffer = this.ledBuffer.slice(
      Dotstar.startBytesLength,
      -Dotstar.endBytesLength,
    );
    // this.clear();
  ...
  set(led: number, r: number, g: number, b: number, a: number = 1) {
    ...
    if (led > this.length) {
      throw new Error(
        `led value must not be greater than the maximum length of the led strip. The max length is: ${this.length}. You passed: ${led}`,
      );
    }
   ...
   */
  private convertRgbaToLedBuffer(
    r: number,
    g: number,
    b: number,
    a: number = 1,
    ...
    const ledBuffer = Buffer.alloc(Dotstar.bytesPerLed);
