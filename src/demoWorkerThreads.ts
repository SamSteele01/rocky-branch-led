...
bluetoothWorker.postMessage({ hereIsYourPort: bluetoothChannel.port1 }, [
...
const sensorInputsWorker = new Worker('./sensors/sensorTestWorker.js')
sensorInputsWorker.on('online', () =>
  console.log(`Sensors process: ${sensorInputsWorker.threadId}`)
)
const sensorChannel = new MessageChannel()
sensorInputsWorker.postMessage({ hereIsYourPort: sensorChannel.port1 }, [
  sensorChannel.port1,
])
// receive message
